# Discover UpDoc engineering roles

Read `knowledge/roles.md`, then inspect the configured UpDoc careers page at
`https://jobs.ashbyhq.com/updoc` (or its underlying job-board feed).

Return only current postings that:

- belong to UpDoc at `updoc.ai`, hosted on `jobs.ashbyhq.com/updoc` exactly;
- are Senior Software Engineer or Senior Software Engineer, AI roles;
- meet the configured location and `discover_from` constraints;
- have a canonical UpDoc/Ashby job URL.
