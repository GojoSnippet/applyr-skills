# Apply to one UpDoc role

Read `knowledge/roles.md` and `knowledge/company.md`. Open only the canonical
job URL supplied by the job record. Process exactly one UpDoc job at a time;
do not open or advance a second job until this one reaches a terminal or
paused state.
...
5. Draft free-text answers from approved candidate evidence plus the
   ...
   authorization, location, motivation, or clinical/healthcare-domain
   background.
6. Pause and ask the user only when a required candidate fact is missing, a
   login/OTP is required, or a CAPTCHA appears.
...
8. Immediately before submitting, re-check the shared durable database for
   ...
   the same job.
9. Submit with `submit_application` only when the form is complete, verified,
   and safe. Record durable confirmation evidence before reporting success.

Never use LinkedIn, bypass a CAPTCHA, apply to a different employer, apply to
an UpDoc role outside the two configured titles, or submit a duplicate.
