# Apply to one Anthropic role

Read `knowledge/roles.md` and `knowledge/company.md`. Open only the canonical
job URL supplied by the job record. Process exactly one Anthropic job at a
...
5. Draft free-text answers from approved candidate evidence plus the
   ...
   authorization, location, motivation, or AI-research-domain background the
   candidate does not actually have.
6. Pause and ask the user only when a required candidate fact is missing, a
   login/OTP is required, or a CAPTCHA/bot-check appears. Never attempt to
   solve or bypass one; pause and flag it instead.
