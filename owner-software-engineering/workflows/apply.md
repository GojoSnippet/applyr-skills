# Apply to one Owner.com role

Read `knowledge/roles.md` and `knowledge/company.md`. Open only the canonical
job URL supplied by the job record. Process exactly one Owner.com job at a
...
5. Draft free-text answers from approved candidate evidence plus the
   ...
   authorization, location, motivation, or restaurant/hospitality-domain
   background.
...
an Owner.com role outside the configured titles, or submit a duplicate.
